...
function loadVideo(server: serverType, vid: number) {
    if (!isRoadblockCalled) {
        roadblock()
        return
    }
...
function getQualityDataHTML5(data: ServerResponse): EpisodeVideoData[] {
    ...
    return videoSources.map((match) => {
        ...
        return {
            url,
            quality,
            expires: parseExpiryFromUrl(url) || undefined,
        } as EpisodeVideoData
    })
}
...
function loadHTML5Player(data: ServerResponse, qualityData?: EpisodeVideoData[]) {
    ...
    let element =
        (document.querySelector('#VideoPlayer') as HTMLDivElement) ||
        (document.querySelector('.gen-video-holder') as HTMLDivElement)
    ...
    Player.download = () => {
        if (downloadCooldown) {
            ...
            Toast.warning('Kérlek várj egy kicsit, mielőtt letöltesz egy videót.', '', {
                duration: 3000,
            })
            return
        }
...
function loadHLSPlayer(data: ServerResponse) {
    parseVideoData(atob(data.hls_url)).then((videoData) => {
        ...
        Player.download = () => {
            ...
            if (videoData[0].url.includes('magyaranime'))
                Toast.warning(
                    'A videó letöltése lassú lesz!',
...
function nextEpisode() {
    ...
    if (nextEpID === -1) {
        ...
        Toast.warning('Nincs következő rész. Átirányítás az anime adatlapra...', '', {
            duration: 3000,
        })
        setTimeout(() => {
...
function silentLoadVideo(server: serverType, vid: number, retryAttempt: number = 0) {
    ...
    getServerResponse(server, vid)
        ...
        .then((nextEpID) => {
            if (nextEpID !== false)
                Toast.success(
                    `Most nézed: ${MA.EPISODE.getEpisodeNumber()}. rész ${nextEpID === -1 ? '(Utolsó)' : ''}`,
                    '',
        ...
        if (prevButton) {
            prevButton.onclick = () => {
                ...
                const prevButton = document.querySelector(
                    ".videoChange[data-vid='" + data.button_vid_prev + "']",
                ) as HTMLButtonElement
                if (prevButton) {
        ...
        if (nextButton) {
            nextButton.onclick = () => {
                ...
                const nextButton = document.querySelector(
                    ".videoChange[data-vid='" + data.button_vid_next + "']",
                ) as HTMLButtonElement
                if (nextButton) {
  ...
  `
    ...
    serverList.querySelectorAll('.mat-server-btn').forEach((button) => {
        button.addEventListener('click', (event) => {
            ...
            const server = target.getAttribute('data-server') as serverType
            ...
            chrome.runtime
                ...
                .then((res) => {
                    if (res) {
                        ...
                        Toast.error('Nem sikerült megnyitni a beállításokat.', '', {
                            duration: 3000,
                        })
                    }
                })
